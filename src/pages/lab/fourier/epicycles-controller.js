...
export default class EpicyclesController extends CanvasController {
	...
	setPath(path, numPoints = -1, minAmplitude = 0.01) {
		...
		this.fourierData = getFourierData(resample2dData(path, this.numPoints)).filter(
			(f) => f.amplitude > minAmplitude
		);
	...
	addToPath() {
		...
		for (let i = 0; i < numFouriers; i++) {
			...
			const angle =
				2 * Math.PI * this.fourierData[i].freq * this.niceAnimAmt + this.fourierData[i].phase;
	...
	renderCircles() {
		...
		for (let i = 0; i < numFouriers; i++) {
			const amplitude = this.fourierData[i].amplitude;
			const angle =
				2 * Math.PI * this.fourierData[i].freq * this.animAmt + this.fourierData[i].phase;
			runningX += amplitude * Math.cos(angle);
			runningY += amplitude * Math.sin(angle);
			if (i == 0) {
