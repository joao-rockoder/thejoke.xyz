...
export default class WaveFrequenciesController extends CanvasController {
	...
	renderWaves() {
		...
		for (let i = 0; i < this.fourierData.length; i++) {
			...
			const wave = getWave(
				(t) => waveData.amplitude * Math.cos(2 * Math.PI * waveData.freq * t + waveData.phase)
			);
	...
	renderLabel() {
		...
		const waveValue =
			waveData.amplitude * Math.cos(2 * Math.PI * waveData.freq * xAmt + waveData.phase);
		let waveTop = 0;
		...
		const yAmt =
			(1 / this.totalHeight) *
			(waveTop + waveData.amplitude + this.waveSpacingMultiple * waveValue);
